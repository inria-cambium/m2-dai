...
Definition GenerateIndp_proof (kername : kername) (ty :  mutual_inductive_body) : Result term :=
  ...
  let aux (e:infolocal) (b:list constructor_body) (j:nat) (t:infolocal -> Result term) : Result term :=
    let auxctr (i:nat) (ctr:constructor_body) (e:infolocal): Result term :=
      ...
      let auxarg arg (t:infolocal->Result term) :infolocal -> Result term := fun e =>
        ...
        (*****************************************)
        | tProd na _ _ =>
          &let t2 := check_return_type t1 e in
          match t2 with
  ...
  let aux' : infolocal -> Nat.t -> Nat.t -> constructor_body -> (context * (infolocal -> Result term)) :=
    fun e j i b =>
    (b.(cstr_args),
     fun e =>
      tApp' (geti_info (String.append "Pctr" (string_of_nat j)) e i) $
        ...
        List.concat
          (let auxarg (arg:context_decl) (e:infolocal) : list (Result term) :=
            ...
            | tProd na _ _ =>
              ...
              | None => [ arg_current ]
              | Some _ =>
                let fix aux_nested e t1 : Result term :=
                  match t1 with
                  | tProd na ta tb =>
  ...
  let generate_fix e j mainbody : def (Result term) :=
